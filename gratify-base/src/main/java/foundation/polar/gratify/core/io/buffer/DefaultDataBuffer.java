...
public class DefaultDataBuffer implements DataBuffer {
   ...
   private DefaultDataBuffer(DefaultDataBufferFactory dataBufferFactory, ByteBuffer byteBuffer) {
      AssertUtils.notNull(dataBufferFactory, "DefaultDataBufferFactory must not be null");
      AssertUtils.notNull(byteBuffer, "ByteBuffer must not be null");
   ...
   public int indexOf(IntPredicate predicate, int fromIndex) {
      AssertUtils.notNull(predicate, "IntPredicate must not be null");
      if (fromIndex < 0) {
         fromIndex = 0;
      }
   ...
   public DefaultDataBuffer read(byte[] destination) {
      AssertUtils.notNull(destination, "Byte array must not be null");
   ...
   public DefaultDataBuffer write(byte[] source) {
      AssertUtils.notNull(source, "Byte array must not be null");
   ...
   private int calculateCapacity(int neededCapacity) {
      AssertUtils.isTrue(neededCapacity >= 0, "'neededCapacity' must >= 0");

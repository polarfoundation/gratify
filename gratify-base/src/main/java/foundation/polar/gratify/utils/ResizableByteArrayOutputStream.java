 ...
 * <ul>
 * <li>has public {@link foundation.polar.gratify.utils.ResizableByteArrayOutputStream#grow(int)}
 * and {@link foundation.polar.gratify.utils.ResizableByteArrayOutputStream#resize(int)} methods
...
public class ResizableByteArrayOutputStream extends ByteArrayOutputStream {
   ...
   public synchronized void resize(int targetCapacity) {
      AssertUtils.isTrue(targetCapacity >= this.count, "New capacity must not be smaller than current size");
   ...
   public synchronized void grow(int additionalCapacity) {
      AssertUtils.isTrue(additionalCapacity >= 0, "Additional capacity must be 0 or higher");

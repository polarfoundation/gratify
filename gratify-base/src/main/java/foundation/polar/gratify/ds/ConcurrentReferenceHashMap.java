...
public class ConcurrentReferenceHashMap<K, V> extends AbstractMap<K, V> implements ConcurrentMap<K, V> {
   ...
   public ConcurrentReferenceHashMap(
      ...
      AssertUtils.isTrue(initialCapacity >= 0, "Initial capacity must not be negative");
      AssertUtils.isTrue(loadFactor > 0f, "Load factor must be positive");
      AssertUtils.isTrue(concurrencyLevel > 0, "Concurrency level must be positive");
      AssertUtils.notNull(referenceType, "Reference type must not be null");
   ...
   private V put(@Nullable final K key, @Nullable final V value, final boolean overwriteExisting) {
      return doTask(key, new Task<V>(TaskOption.RESTRUCTURE_BEFORE, TaskOption.RESIZE) {
         ...
         protected V execute(@Nullable Reference<K, V> ref, @Nullable Entry<K, V> entry, @Nullable Entries<V> entries) {
            ...
            AssertUtils.state(entries != null, "No entries segment");
            entries.add(value);
            return null;
         }
      });
   ...
   private class EntryIterator implements Iterator<Map.Entry<K, V>> {
      ...
      @Override
      public void remove() {
         AssertUtils.state(this.last != null, "No element to remove");

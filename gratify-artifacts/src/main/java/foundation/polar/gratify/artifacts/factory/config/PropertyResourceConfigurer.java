...
/**
 * Allows for configuration of individual artifact property values from a property resource,
 * i.e. a properties file. Useful for custom config files targeted at system
 * administrators that override artifact properties configured in the application context.
 *
 * <p>Two concrete implementations are provided in the distribution:
 * <ul>
 * <li>{@link PropertyOverrideConfigurer} for "artifactName.property=value" style overriding
 * (<i>pushing</i> values from a properties file into artifact definitions)
 * <li>{@link PropertyPlaceholderConfigurer} for replacing "${...}" placeholders
 * (<i>pulling</i> values from a properties file into artifact definitions)
 * </ul>
 *
...
public abstract class PropertyResourceConfigurer extends PropertiesLoaderSupport
   implements ArtifactFactoryPostProcessor, PriorityOrdered {
    ...
    * {@linkplain #processProperties process} properties against the given artifact factory.
    * @throws ArtifactInitializationException if any properties cannot be loaded
    */
   @Override
   public void postProcessArtifactFactory(ConfigurableListableArtifactFactory artifactFactory) throws ArtifactsException {
      try {
         ...
         processProperties(artifactFactory, mergedProps);
      }
      catch (IOException ex) {
         throw new ArtifactInitializationException("Could not load properties", ex);
      }
   }
   ...
   /**
    * Apply the given Properties to the given ArtifactFactory.
    * @param artifactFactory the ArtifactFactory used by the application context
    * @param props the Properties to apply
    * @throws foundation.polar.gratify.artifacts.ArtifactsException in case of errors
    */
   protected abstract void processProperties(ConfigurableListableArtifactFactory artifactFactory, Properties props)

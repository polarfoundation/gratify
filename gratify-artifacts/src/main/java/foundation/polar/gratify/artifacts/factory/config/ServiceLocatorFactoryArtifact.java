...
/**
 * A {@link FactoryArtifact} implementation that takes an interface which must have one or more
 ...
 * underlying {@link foundation.polar.gratify.artifacts.factory.ArtifactFactory}.
 *
 * <p>Such service locators permit the decoupling of calling code from
 * the {@link foundation.polar.gratify.artifacts.factory.ArtifactFactory} API, by using an
 * appropriate custom locator interface. They will typically be used for
 * <b>prototype artifacts</b>, i.e. for factory methods that are supposed to
 ...
 * the locator's factory methods on demand. <b>For singleton artifacts, direct
 * setter or constructor injection of the target artifact is preferable.</b>
 *
 ...
 * <b>one</b> artifact in the factory matches the return type of the factory
 * method, that artifact is returned, otherwise a
 * {@link foundation.polar.gratify.artifacts.factory.NoSuchArtifactDefinitionException}
 * is thrown.
 *
 * <p>On invocation of the single-arg factory method with a non-null (and
 * non-empty) argument, the proxy returns the result of a
 * {@link foundation.polar.gratify.artifacts.factory.ArtifactFactory#getArtifact(String)} call,
 * using a stringified version of the passed-in id as artifact name.
 *
 ...
 * {@code toString}. The resulting String can be used as artifact name as-is,
 * provided that corresponding artifacts are defined in the artifact factory.
 * Alternatively, {@linkplain #setServiceMappings(java.util.Properties) a custom
 * mapping} between service IDs and artifact names can be defined.
 *
 * <p>By way of an example, consider the following service locator interface.
 * Note that this interface is not dependent on any Gratify APIs.
 *
 ...
 *
 *   &lt;!-- Prototype artifact since we have state -->
 *   &lt;artifact id="myService" class="a.b.c.MyService" singleton="false"/>
 *
 *   &lt;!-- will lookup the above 'myService' artifact by *TYPE* -->
 *   &lt;artifact id="myServiceFactory"
 *            class="foundation.polar.gratify.artifacts.factory.config.ServiceLocatorFactoryArtifact">
 *     &lt;property name="serviceLocatorInterface" value="a.b.c.ServiceFactory"/>
 *   &lt;/artifact>
 *
 *   &lt;artifact id="clientArtifact" class="a.b.c.MyClientArtifact">
 *     &lt;property name="myServiceFactory" ref="myServiceFactory"/>
 *   &lt;/artifact>
 *
 *&lt;/artifacts></pre>
 *
 * <p>The attendant {@code MyClientArtifact} class implementation might then
 ...
 *
 *    // actual implementation provided by the Gratify container
 ...
 *}</pre>
 *
 * <p>By way of an example that looks up a artifact <b>by name</b>, consider
 * the following service locator interface. Again, note that this
 * interface is not dependent on any Gratify APIs.
 *
 * <pre class="code">package a.b.c;
 *
 *public interface ServiceFactory {
 ...
 * <pre class="code">&lt;artifacts>
 *
 *   &lt;!-- Prototype artifacts since we have state (both extend MyService) -->
 *   &lt;artifact id="specialService" class="a.b.c.SpecialService" singleton="false"/>
 *   &lt;artifact id="anotherService" class="a.b.c.AnotherService" singleton="false"/>
 *
 *   &lt;artifact id="myServiceFactory"
 *            class="foundation.polar.gratify.artifacts.factory.config.ServiceLocatorFactoryArtifact">
 *     &lt;property name="serviceLocatorInterface" value="a.b.c.ServiceFactory"/>
 ...
 *}</pre>
 *
 * <p>See {@link ObjectFactoryCreatingFactoryArtifact} for an alternate approach.
 *
 ...
 */
public class ServiceLocatorFactoryArtifact
   implements FactoryArtifact<Object>, ArtifactFactoryAware, InitializingArtifact {
   @Nullable
   ...
   @Nullable
   private ListableArtifactFactory artifactFactory;
    ...
    * (typically, {@code MyService getService()} or {@code MyService getService(String id)}).
    * See the {@link ServiceLocatorFactoryArtifact class-level Javadoc} for
    ...
    * <p>If not specified, subclasses of Gratify's ArtifactsException will be thrown,
    * for example NoSuchArtifactDefinitionException. As those are unchecked, the
    * caller does not need to handle them, so it might be acceptable that
    * Gratify exceptions get thrown as long as they are just handled generically.
    ...
    * and artifact names (in the artifact factory). Service ids that are not defined
    * here will be treated as artifact names as-is.
    ...
    * a single matching artifact will be retrieved from the artifact factory.
    * @param serviceMappings mappings between service ids and artifact names,
    * with service ids as keys as artifact names as values
    */
   ...
   @Override
   public void setArtifactFactory(ArtifactFactory artifactFactory) throws ArtifactsException {
      if (!(artifactFactory instanceof ListableArtifactFactory)) {
         throw new FatalArtifactException(
            "ServiceLocatorFactoryArtifact needs to run in a ArtifactFactory that is a ListableArtifactFactory");
      }
      this.artifactFactory = (ListableArtifactFactory) artifactFactory;
   }
    ...
    * @see #setServiceLocatorExceptionClass
    */
   protected Exception createServiceLocatorException(Constructor<Exception> exceptionConstructor, ArtifactsException cause) {
      ...
      return ArtifactUtils.instantiateClass(exceptionConstructor, args);
   }
   ...
   /**
    * Invocation handler that delegates service locator calls to the artifact factory.
    */
   private class ServiceLocatorInvocationHandler implements InvocationHandler {
      ...
      private Object invokeServiceLocatorMethod(Method method, Object[] args) throws Exception {
         ...
         try {
            String artifactName = tryGetArtifactName(args);
            AssertUtils.state(artifactFactory != null, "No ArtifactFactory available");
            if (StringUtils.hasLength(artifactName)) {
               // Service locator for a specific artifact name
               return artifactFactory.getArtifact(artifactName, serviceLocatorMethodReturnType);
            }
            else {
               // Service locator for a artifact type
               return artifactFactory.getArtifact(serviceLocatorMethodReturnType);
            }
         }
       ...
       */
      private String tryGetArtifactName(@Nullable Object[] args) {
         String artifactName = "";
         if (args != null && args.length == 1 && args[0] != null) {
            artifactName = args[0].toString();
         }
         // Look for explicit serviceId-to-artifactName mappings.
         if (serviceMappings != null) {
            String mappedName = serviceMappings.getProperty(artifactName);
            if (mappedName != null) {
               artifactName = mappedName;
            }
         }
         return artifactName;
      }
      ...
      private Class<?> getServiceLocatorMethodReturnType(Method method) throws NoSuchMethodException {
         AssertUtils.state(serviceLocatorInterface != null, "No service locator interface specified");
